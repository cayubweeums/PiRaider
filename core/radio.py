...
# Frequency ranges (MHz): list of (low_mhz, high_mhz, band_label) for 2.4/5/6 GHz bands.
...
def grab_all_wireless_interfaces() -> dict | None:
    """
    Grabs all wireless interfaces, pulls relevant info for them like state and capabilities.

    Returns:
        dict | None: Dict of wireless interfaces and their info (device_id, physical_id,
            interface_up, band_capability). Empty dict on error.
    """
    ...
    if result.returncode != 0:
        log.error(f"Failed to list wireless interfaces: {result.stderr}")
        return relevant_device_info
    else:
        interfaces = []
...
def grab_all_bluetooth_interfaces() -> dict:
    ...
    Returns:
        dict: Dict of bluetooth controllers and their info. Empty dict on error.
    """
    relevant_device_info = {}
